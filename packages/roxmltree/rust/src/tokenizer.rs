...
use crate::error::{TextPos, XmlError};
...
pub fn parse<'input>(
    ...
    if s.starts_with(b"<!DOCTYPE") {
        if !allow_dtd {
            return Err(XmlError::DtdDetected);
        }
    ...
    if !s.at_end() {
        return Err(XmlError::UnknownToken {
            pos: s.gen_text_pos(),
        });
    }
...
fn parse_declaration(s: &mut Stream) -> Result<()> {
    fn consume_spaces(s: &mut Stream) -> Result<()> {
        if s.starts_with_space() {
            ...
            return Err(XmlError::InvalidChar2 {
                expected: "a whitespace",
                actual: s.curr_byte_unchecked(),
                pos: s.gen_text_pos(),
            });
        }
...
fn parse_pi<'input>(s: &mut Stream<'input>, events: &mut dyn XmlEvents<'input>) -> Result<()> {
    if s.starts_with(b"<?xml ") {
        return Err(XmlError::UnexpectedDeclaration {
            pos: s.gen_text_pos(),
        });
    }
    ...
    let range = s.range_from(start);
    events.token(Token::ProcessingInstruction {
        target,
        content,
        range,
    })?;
    ...
    while !s.at_end() {
        ...
        if s.starts_with(b"<!ENTITY") {
            ...
            match s.curr_byte() {
                ...
                Err(_) => {
                    return Err(XmlError::UnexpectedEndOfStream);
                }
            }
            ...
            || s.starts_with(b"<!NOTATION")
        {
            if consume_decl(s).is_err() {
                ...
                return Err(XmlError::UnknownToken { pos });
            }
        } else {
            return Err(XmlError::UnknownToken {
                pos: s.gen_text_pos(),
    ...
    match c {
        ...
        b'S' | b'P' => {
            if parse_external_id(s)? {
                ...
                Err(XmlError::InvalidExternalID {
                    pos: s.gen_text_pos(),
                })
            }
        }
...
fn parse_element<'input>(s: &mut Stream<'input>, events: &mut dyn XmlEvents<'input>) -> Result<()> {
    ...
    events.token(Token::ElementStart {
        prefix,
        local,
        start,
    })?;
...
fn parse_text<'input>(s: &mut Stream<'input>, events: &mut dyn XmlEvents<'input>) -> Result<()> {
    ...
    if text.contains('>') && text.contains("]]>") {
        return Err(XmlError::InvalidCharacterData {
            pos: s.gen_text_pos(),
        });
    }
...
impl<'input> Stream<'input> {
    ...
    fn consume_byte(&mut self, c: u8) -> Result<()> {
        ...
        if curr != c {
            return Err(XmlError::InvalidChar {
                expected: c,
                actual: curr,
    ...
    fn skip_string(&mut self, text: &'static [u8]) -> Result<()> {
        if !self.starts_with(text) {
            ...
            return Err(XmlError::InvalidString { expected, pos });
        }
        ...
        F: Fn(&Stream, char) -> bool,
    {
        for c in self.chars() {
            if !c.is_xml_char() {
                return Err(XmlError::NonXmlChar {
                    char: c,
                    pos: self.gen_text_pos(),
                });
            } else if f(self, c) {
    ...
    fn consume_spaces(&mut self) -> Result<()> {
        ...
        if !self.starts_with_space() {
            return Err(XmlError::InvalidChar2 {
                expected: "a whitespace",
                actual: self.curr_byte_unchecked(),
                pos: self.gen_text_pos(),
            });
        }
    ...
    fn consume_name(&mut self) -> Result<&'input str> {
        ...
        if name.is_empty() {
            return Err(XmlError::InvalidName {
                pos: self.gen_text_pos_from(start),
            });
        }
