...
/**
 * Creates a fetch client with OpenAPI-typed REST helpers already installed.
 * Pass the `paths` type generated by `openapi-typescript` as the generic argument.
 */
 ...
 * Unknown paths, unsupported methods, missing params, and invalid bodies fail at compile time.
 */
...
/**
 * Sends one OpenAPI-typed request for a specific HTTP method.
 *
 * The path, params, headers, body, success data, and HTTP error data are inferred
 * from the matching OpenAPI operation. Pass `withResponse: true` to receive
 * `{ data, response }` on the success branch.
 */
...
/** Tuple-result response inferred from one OpenAPI operation. */
