...
import { withTimestamp } from './with-timestamp-prefix';

describe('withTimestampPrefix function', () => {
	...
	it('should add timestamp middleware correctly', () => {
		...
		expect(timestampedLogger._features.includes('timestamp-prefix')).toBe(true);
		expect(timestampedLogger.middlewares.length).toBe(1);
	});
	...
	it('should prepend timestamp to log messages', () => {
		...
		expect(consoleSpies.trace).toHaveBeenCalledWith(`[${mockDate.toLocaleString()}] trace message`);

		timestampedLogger.debug('debug message');
		expect(consoleSpies.debug).toHaveBeenCalledWith(`[${mockDate.toLocaleString()}] debug message`);
		...
		expect(consoleSpies.info).toHaveBeenCalledWith(`[${mockDate.toLocaleString()}] info message`);

		timestampedLogger.warn('warn message');
		expect(consoleSpies.warn).toHaveBeenCalledWith(`[${mockDate.toLocaleString()}] warn message`);

		timestampedLogger.error('error message');
		expect(consoleSpies.error).toHaveBeenCalledWith(`[${mockDate.toLocaleString()}] error message`);
	});
});
