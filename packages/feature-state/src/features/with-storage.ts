	...
	return <GValue, GFeatures extends TFeatureDefinition[]>(
		...
		const persistFeature: TPersistFeature['api'] = {
			async persist() {
				...
				if (!success) {
					success = await storage.save(key, state._v as unknown as GStorageValue);
				}
			...
			async loadFormStorage() {
				...
				if (persistedValue !== FAILED_TO_LOAD_FROM_STORAGE_IDENTIFIER) {
					state.set(persistedValue as unknown as GValue, {
						additionalData: { source: LOAD_FROM_STORAGE_SOURCE_KEY }
					});
					success = true;
				}
