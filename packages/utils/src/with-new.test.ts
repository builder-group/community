...
import { withNew, withNewAsync } from './with-new';
...
describe('withNewAsync function', () => {
	it('should initialize object without arguments', async () => {
		const testObj = {
			value: 0,
			async _new() {
		...
		const result = await withNewAsync(testObj);
		expect(result.value).toBe(42);
		expect('_new' in result).toBe(false);
	});

	it('should initialize object with a single argument', async () => {
		const testObj = {
			value: 0,
			async _new(newValue: number) {
				await Promise.resolve();
				this.value = newValue;
			}
		};

		const result = await withNewAsync(testObj, 123);
		expect(result.value).toBe(123);
		expect('_new' in result).toBe(false);
	});

	it('should initialize object with multiple arguments', async () => {
		type TTestObj = {
			name: string;
			age: number;
			active: boolean;
			_new(name: string, age: number, active: boolean): Promise<void>;
		};
		...
		const testObj: TTestObj = {
			...
			age: 0,
			active: false,
			async _new(name: string, age: number, active: boolean) {
				await Promise.resolve();
				this.name = name;
				this.age = age;
				this.active = active;
		...
		const result = await withNewAsync(testObj, 'John', 30, true);
		expect(result.name).toBe('John');
		expect(result.age).toBe(30);
		expect(result.active).toBe(true);
		expect('_new' in result).toBe(false);
	...
	it('should return object as is if no new function present', async () => {
		const testObj = {
			value: 42
		} as any;
	...
	it('should preserve object structure after initialization', async () => {
		const testObj = {
			nested: { value: 0 },
			async _new() {
				await Promise.resolve();
